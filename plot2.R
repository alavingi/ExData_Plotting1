...
plot(as.POSIXct(strptime(paste(as.character(data$Date), " ", as.character(data$Time)), format = "%Y-%m-%d %H:%M:%S", tz = "")), as.numeric(as.character(data$Global_active_power)), type = "l", ylab = "Global Active Power (kilowatts)", xlab = "")
